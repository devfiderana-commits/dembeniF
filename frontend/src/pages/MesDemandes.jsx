...
const MesDemandes = () => {
  ...
  const displayedDemandes = filter === 'tous'
    ? demandes
    : demandes.filter((d) => d.status === filter);

  const getStatusInfo = (s) => {
    switch(s) {
      case 'pending': return { label: 'En attente', color: 'bg-amber-50 text-amber-600 border-amber-100', icon: Clock };
      case 'approved': return { label: 'Approuvée', color: 'bg-emerald-50 text-emerald-600 border-emerald-100', icon: CheckCircle };
      case 'rejected': return { label: 'Rejetée', color: 'bg-red-50 text-red-600 border-red-100', icon: XCircle };
      default: return { label: s || 'Inconnu', color: 'bg-slate-50 text-slate-500 border-slate-100', icon: AlertCircle };
    }
  };
                 ...
                 return (
                    ...
                    >
                       <div className={`w-16 h-16 rounded-[1.5rem] shrink-0 group-hover:scale-110 transition-transform flex items-center justify-center text-3xl ${
                          d.type === 'identite' ? 'bg-indigo-50 text-indigo-600' :
                          d.type === 'etat-civil' ? 'bg-emerald-50 text-emerald-600' :
                       ...
                       <div className="flex-1 text-center md:text-left">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-2">REF: {d._id?.substring(0, 8)}</p>
                          <h3 className="text-xl font-black text-slate-900 mb-3 group-hover:text-green-700 transition-colors">{d.title}</h3>
                          <div className="flex flex-wrap items-center justify-center md:justify-start gap-6">
                             ...
                             <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                <Clock className="w-4 h-4 text-slate-300" /> {d.attachmentUrl ? 1 : 0} document(s) joint(s)
                             </div>
                          </div>
                       </div>
