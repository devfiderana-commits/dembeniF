...
const AdminDemandes = () => {
  ...
  const fetchDemandes = async () => {
    try {
      const res = await adminAPI.getDemandes();
  ...
  const handleUpdateStatus = async (id, status) => {
    try {
      await adminAPI.respondToDemande(id, { status, message: comment });
  ...
  const filteredDemandes = demandes.filter(d => {
    const citizenName = `${d.citizenId?.firstname || ''} ${d.citizenId?.lastname || ''}`.trim().toLowerCase();
    const title = (d.title || d.type || '').toLowerCase();
    return (
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                filteredDemandes.map(d => {
                  const citizenName = `${d.citizenId?.firstname || ''} ${d.citizenId?.lastname || ''}`.trim() || 'Citoyen';
                  return (
                    <tr key={d._id} className="hover:bg-gray-50/60 transition-colors cursor-pointer group">
                      <td className="px-6 py-4 font-mono text-sm font-bold text-blue-600">{d._id?.substring(0, 8)}</td>
                      <td className="px-6 py-4">
                         <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-lg bg-blue-50 flex items-center justify-center text-blue-600 text-xs font-bold uppercase">{citizenName.charAt(0)}</div>
                            <div>
                              <p className="text-sm font-bold text-gray-800">{citizenName}</p>
                              <p className="text-[10px] text-gray-400">{d.citizenId?.email || '—'}</p>
                            </div>
                         </div>
                      </td>
                      <td className="px-6 py-4">
                         <p className="text-sm font-semibold text-gray-700">{d.title || d.type || 'Demande'}</p>
                         <p className="text-[10px] text-gray-400 capitalize">{d.type || 'Non spécifié'}</p>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                         {format(new Date(d.createdAt), 'dd MMM yyyy', { locale: fr })}
                      </td>
                      <td className="px-6 py-4">
                         <span className={`px-3 py-1 rounded-full text-[10px] font-bold border uppercase ${getBadgeStyle(d.status)}`}>
                            {d.status?.replace('_', ' ') || 'Inconnu'}
                         </span>
                      </td>
                      <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-y-auto">
           ...
           <div className="bg-white rounded-[2rem] w-full max-w-2xl shadow-2xl relative fade-in max-h-[90vh] flex flex-col">
              <div className="p-8 border-b border-gray-100 flex items-start justify-between shrink-0">
                 <div>
                    <span className="text-[10px] font-bold text-gray-400 tracking-widest uppercase mb-1 block">Détails de l'instruction — {selectedDemande._id?.substring(0, 8)}</span>
                    <h2 className="text-xl font-bold text-gray-900 font-['Poppins']">{selectedDemande.title || selectedDemande.type || 'Demande'}</h2>
                 </div>
              ...
              <div className="p-8 space-y-6 overflow-y-auto">
                    ...
                    <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                       ...
                       <div className="flex items-center gap-3">
                          <Clock className="w-4 h-4 text-blue-600" />
                               <p className="text-sm font-bold">{`${selectedDemande.citizenId?.firstname || ''} ${selectedDemande.citizenId?.lastname || ''}`.trim() || 'Citoyen'}</p>
                            </div>
                         </div>
                         <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
                            <p className="text-[10px] font-bold text-gray-400 uppercase mb-2">Statut Actuel</p>
                            <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold border uppercase ${getBadgeStyle(selectedDemande.status)}`}>
                              {selectedDemande.status?.replace('_', ' ') || 'Inconnu'}
                            </span>
                         </div>
                      </div>
                 ...
                 <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {selectedDemande.attachmentUrl ? (
                        <a 
                          href={selectedDemande.attachmentUrl.startsWith('http') ? selectedDemande.attachmentUrl : `/uploads/${selectedDemande.attachmentUrl}`} 
                          target="_blank" 
                          rel="noreferrer"
                        ...
                        >
                           ...
                           <span className="text-xs font-bold text-gray-700 truncate">Pièce jointe</span>
                           <ExternalLink className="w-3 h-3 text-gray-300 ml-auto" />
                        </a>
                      ) : (
                        <p className="text-xs text-gray-400 italic">Aucune pièce jointe fournie</p>
                      )}
                      <textarea 
                         value={comment}
              ...
              <div className="p-8 border-t border-gray-100 bg-gray-50/50 flex flex-wrap gap-3 shrink-0 rounded-b-[2rem]">
                 <button 
                   onClick={() => handleUpdateStatus(selectedDemande._id, 'approved')}
                 ...
                 <button 
                    onClick={() => handleUpdateStatus(selectedDemande._id, 'rejected')}
